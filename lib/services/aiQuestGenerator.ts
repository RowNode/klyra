...
const questPrompt = ChatPromptTemplate.fromMessages([
  [
    "system",
    `You are an AI quest designer for a DeFi quest platform on Mantle Sepolia. 
Generate concrete, verifiable quests that revolve around on-chain activity. 
Return only valid JSON that matches the required schema.

Available protocol on Mantle Sepolia:
- Agni DEX (0xb5Dc27be0a565A4A80440f41c74137001920CB22): Decentralized exchange (DEX) for token swaps. Website: https://agniswap.fi/

When generating quests, consider the protocol's features and make quests actionable and verifiable.`,
  ],
  [
    "human",
    `Project: {projectName}
...
Reward amount: {rewardAmount} KLYRA
...
export async function generateQuestWithGroq(rawInput: unknown) {
  ...
  let protocolInfo = `${protocol.name} - ${protocol.description}. Category: ${protocol.category}. Website: ${protocol.website}`
  
  // Add available tokens for swap protocols
  if (protocol.category === "swap" && protocol.swapTokens && protocol.swapTokens.length > 0) {
    const tokensList = protocol.swapTokens.map(t => `${t.symbol} (${t.address})`).join(", ")
    protocolInfo += `. Available tokens for swap: ${tokensList}. You should specify which tokens to use in the quest requirements.`
  }
