...
export interface OnChainReward {
  questId: number
  klyraAmount: string // Amount in wei (18 decimals)
...
/**
 * Get all rewards (KLYRA + badges) for a user from database (already tracked in user_xp_ledger)
...
export async function getUserRewardsFromChain(walletAddress: string): Promise<OnChainReward[]> {
  ...
  try {
    ...
    for (const entry of xpEntries) {
      ...
      // reward_amount is stored as NUMERIC in database (KLYRA amount, not wei)
      ...
      if (badgeLevel) {
        ...
        const reward: OnChainReward = {
          ...
          klyraAmount: rewardAmountStr, // This is in KLYRA (not wei) from database
          badgeLevel,
        ...
        // KLYRA reward without badge
        rewards.push({
          questId: entry.quest_id_on_chain,
          klyraAmount: rewardAmountStr,
          transactionHash: entry.completion_tx_hash || "",
          timestamp: new Date(entry.created_at).getTime() / 1000,
        })
      }
