...
import { PROTOCOLS } from "@/lib/api-services/protocols"
...
export async function generateDailyQuest(walletAddress: string) {
  ...
  const result = await generateQuestWithGroq({
    ...
    rewardAmount: "50", // Smaller reward for daily (50 KLYRA)
    badgeLevel: 1,
    autoDeploy: true,
...
export async function generateWeeklyQuest(walletAddress: string) {
  ...
  const result = await generateQuestWithGroq({
    ...
    protocol: protocol.evmAddress,
    participant: walletAddress,
    rewardAmount: "200", // Larger reward for weekly (200 KLYRA)
 ...
 * Daily quest: must be created TODAY (same day)
 * Weekly quest: must be created THIS WEEK (starting from Monday)
 */
export async function getUserActiveQuests(walletAddress: string) {
  try {
    const { supabase } = await import("@/lib/api-services/supabase")
    ...
    // Get all daily/weekly quests for this user
    ...
    // Filter daily quests: must be created TODAY
    const dailyQuestsToday = data.filter((q: any) => {
      if (q.quest_type !== "daily") return false
      const createdDate = new Date(q.created_at)
      return createdDate >= todayStart
    })
    const daily = dailyQuestsToday.length > 0 ? dailyQuestsToday[0] : null

    // Filter weekly quests: must be created THIS WEEK (since Monday)
    const weeklyQuestsThisWeek = data.filter((q: any) => {
      if (q.quest_type !== "weekly") return false
      const createdDate = new Date(q.created_at)
      return createdDate >= weekStart
    })
    const weekly = weeklyQuestsThisWeek.length > 0 ? weeklyQuestsThisWeek[0] : null
