...
import { generateUserAvatar } from "@/lib/utils/avatarGenerator"
...
/**
 * Get existing submission if it exists (any status)
 */
...
/**
 * Check if transaction hash already submitted for quest (any status)
 */
export async function isTransactionHashSubmitted(
  questIdOnChain: number,
  txHash: string
): Promise<boolean> {
  const submission = await getExistingSubmission(questIdOnChain, txHash)
  return !!submission
}
...
export async function updateSubmissionStatus(
  ...
  if (mirrorNodePayload !== undefined) {
    updateData.mirror_node_payload = mirrorNodePayload
  }
