...
import {KlyraToken} from "../src/tokens/KlyraToken.sol";
...
/**
 * @title Deploy
 * @notice Comprehensive deployment script for Klyra DeFi Quest system.
 *         Deploys ERC-8004 registries, KLYRA ERC-20 token, Badge ERC-721 NFT, RewardVault, and QuestManager.
...
contract Deploy is Script {
    function run() external {
        ...
        // 5. Deploy KLYRA ERC-20 Token
        console.log("\n=== Deploying KlyraToken (ERC-20) ===");
        vm.startBroadcast(deployerPrivateKey);
        klyraToken = new KlyraToken(deployer);
        vm.stopBroadcast();
        console.log("KlyraToken deployed at:", address(klyraToken));
        ...
        // Grant MINTER_ROLE to RewardVault in KlyraToken
        console.log("Granting MINTER_ROLE to RewardVault...");
        vm.startBroadcast(deployerPrivateKey);
        klyraToken.grantMinterRole(address(rewardVault));
        vm.stopBroadcast();

        // Set KlyraToken in RewardVault
        console.log("Setting KlyraToken in RewardVault...");
        vm.startBroadcast(deployerPrivateKey);
        rewardVault.setKlyraToken(address(klyraToken));
        vm.stopBroadcast();
        ...
        console.log("KlyraToken (ERC-20):", address(klyraToken));
        ...
        console.log("\n=== Next Steps ===");
        console.log("1. Register Agent: forge script script/RegisterAgent.s.sol:RegisterAgent --rpc-url mantle-sepolia --broadcast");
        ...
        console.log("   - REWARD_VAULT_ADDRESS=", address(rewardVault));
        console.log("   - BADGE_NFT_ADDRESS=", address(badgeNft));
        ...
        console.log("   - AGENT_REGISTRY_ADAPTER_ADDRESS=", address(agentRegistryAdapter));
        console.log("   - COMPLETION_ORACLE=", completionOracle);
        console.log("   - COMPLETION_ORACLE_PRIVATE_KEY (must match oracle address)");
        console.log("3. Update frontend .env.local with:");
        console.log("   - NEXT_PUBLIC_QUEST_MANAGER_ADDRESS=", address(questManager));
        console.log("   - NEXT_PUBLIC_KLYRA_TOKEN_ADDRESS=", address(klyraToken));
        console.log("   - NEXT_PUBLIC_BADGE_NFT_ADDRESS=", address(badgeNft));
        console.log("   - NEXT_PUBLIC_REWARD_VAULT_ADDRESS=", address(rewardVault));
    }
}
