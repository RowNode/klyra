...
import {KlyraToken} from "./tokens/KlyraToken.sol";

/**
 * @title RewardVault
 * @notice Holds KLYRA ERC-20 reward supply. The vault mints KLYRA tokens
...
contract RewardVault is Ownable {
    ...
    KlyraToken public klyraToken;

    event QuestManagerUpdated(address indexed newQuestManager);
    event KlyraTokenUpdated(address indexed tokenAddress);
    ...
    error RewardVault__KlyraTokenNotConfigured();
    ...
    function setKlyraToken(address tokenAddress) external onlyOwner {
        if (tokenAddress == address(0)) revert RewardVault__KlyraTokenNotConfigured();
        klyraToken = KlyraToken(tokenAddress);
        emit KlyraTokenUpdated(tokenAddress);
    }

    /**
     * @notice Fund a quest with KLYRA tokens. Mints tokens to this vault.
     *         Called by QuestManager when a quest is created.
     * @param questId ID of the quest to fund
     * @param amount Total amount of KLYRA tokens to fund (with 18 decimals)
     */
    function fundQuest(uint256 questId, uint256 amount) external onlyQuestManager {
        if (address(klyraToken) == address(0)) revert RewardVault__KlyraTokenNotConfigured();
        ...
        klyraToken.mint(address(this), amount);
     ...
     * @param amount Amount of KLYRA tokens to release (with 18 decimals)
     */
    function releaseReward(uint256 questId, address recipient, uint256 amount) external onlyQuestManager {
        ...
        IERC20(address(klyraToken)).safeTransfer(recipient, amount);
